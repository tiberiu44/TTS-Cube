...
class CubedallEncoder(nn.Module):
    def __init__(self, C, D, strides=[4, 4, 5, 6]):
        ...
        self.layers = nn.Sequential(
            ...
            nn.ELU(),
            ConvNorm(in_channels=16 * C, out_channels=D, kernel_size=7, padding=3, w_init_gain='elu'),
            nn.ELU(),
            ConvNorm(in_channels=D, out_channels=D, kernel_size=7, padding=3, w_init_gain='elu'),
            nn.ELU(),
            ConvNorm(in_channels=D, out_channels=D, kernel_size=7, padding=3, w_init_gain='elu'),
            nn.ELU(),
            ConvNorm(in_channels=D, out_channels=D, kernel_size=7, padding=3, w_init_gain='elu'),
